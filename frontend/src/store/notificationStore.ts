...
interface NotificationState {
  ...
  deleteOldestRead: (dept: Department) => AppNotification | null;
...
export const useNotificationStore = create<NotificationState>((set, get) => ({
  ...
  deleteOldestRead(dept) {
    const state = get();
    // 부서 내 '읽음(read)' 처리된 알림만 추출
    const readItems = state.items.filter((n) => n.department === dept && n.read);
    if (readItems.length === 0) return null;
    
    // 가장 오래된 것 찾기
    readItems.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
    const oldest = readItems[0];
    
    // 상태에서 제거
    set((s) => ({ items: s.items.filter((n) => n.id !== oldest.id) }));
    return oldest;
  },
  deleteItem(id) {
  ...
  clearAll() {
    set({ items: [], toasts: [], banners: [], modalQueue: [], seeded: false });
  },
}));
